...
namespace CardGame.GodotClient
{
    ...
    public partial class GameBootstrap : Node2D
    {
        ...
        public override void _Ready()
        {
            if (UI == null)
            {
                GD.PrintErr("CRITICAL: UIManager is missing in GameBootstrap!");
                return;
            }
        ...
        public override void _Process(double delta)
        {
            ...
            // 3. Update Visuals (Only if state changed)
        ...
        private void InitializeGame()
        {
            ...
            try
            {
                CardLibrary.Instance.LoadFromJson(jsonPath);
            }
            catch (Exception ex)
            {
                GD.PrintErr($"[BOOTSTRAP] Failed to load cards: {ex.Message}");
            }
            ...
            {
                GD.Print($"[BOOTSTRAP] Loading selected deck: {GameSession.Instance.SelectedPlayerDeck.Name}");
            ...
            {
                // Fallback for direct scene launch (Debug Mode)
                GD.Print("[BOOTSTRAP] No session data found. Loading latest deck from disk (Debug Mode).");
            ...
            if (d1.Count == 0)
            {
                d1 = CreateRandomDeck(factory, _playerId);
            }
            ...
            if (d2.Count == 0)
            {
                ...
                if (d2.Count == 0)
                {
                    d2 = CreateRandomDeck(factory, _botId);
                }
            }
        ...
        private void OnCardDropped(int cardId, int lineIdx)
        {
            ...
            if (card != null)
                _inputController.HandleCardDrop(card, lineIdx);
        }

        #endregion

        #region Deck Loading Helpers
