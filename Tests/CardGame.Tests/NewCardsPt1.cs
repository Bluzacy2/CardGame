...
namespace CardGame.Tests
{
    public class NewCardsMechanicsTests
    {
        ...
        [Fact]
        public void Informer_ShouldAutoResolve_WhenOnlyOneEnemyExists()
        {
            ...
            engine.ExecuteCommand(new PlayUnitCommand(1, informer.InstanceId, 0));

            var victimOnBoard = engine.CurrentState.Board.Lines[0].Player2Unit;
            Assert.Contains(Keyword.Marked, victimOnBoard!.CurrentStats.Keywords);
            Assert.Null(engine.CurrentState.PendingInteraction);
        }

        [Fact]
        public void FinalMission_ShouldAutoResolve_WhenOnlyOneFriendlyUnitExists()
        {
            var engine = TestHelpers.CreateEngineWithCards(GetFullCardsJson());
        ...
        public void Juggernaut_ShouldReduceDamageByTwo_AndPingHero()
        {
            ...
            var victim = engine.Factory.CreateCard(99, 2);
            engine.CurrentState = engine.CurrentState.UpdateBoard(engine.CurrentState.Board.WithUnitPlacedAt(0, 1, jugg).WithUnitPlacedAt(0, 2, victim));
        ...
        public void Gerard_ShouldBonusAttack_WhenEnemyIsMarked()
        {
            ...
            var victim = engine.Factory.CreateCard(99, 2);
            var informer = engine.Factory.CreateCard(4, 1);
            engine.CurrentState = engine.CurrentState.UpdateBoard(engine.CurrentState.Board.WithUnitPlacedAt(0, 1, gerard).WithUnitPlacedAt(0, 2, victim));
            engine.CurrentState = engine.CurrentState.UpdatePlayer(engine.CurrentState.PlayerA.WithCardAddedToHand(informer));
        ...
        public void FireAxeMan_ShouldHealAndBuff_OnKill()
        {
            ...
            engine.CurrentState = engine.CurrentState.UpdateBoard(engine.CurrentState.Board.WithUnitPlacedAt(0, 1, axeMan).WithUnitPlacedAt(0, 2, victim));
            engine.CurrentState = engine.CurrentState.With(currentPhase: GamePhase.Combat);
            engine.ExecuteCommand(new EndPhaseCommand(1));
